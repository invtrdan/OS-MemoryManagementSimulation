...
// Allocate a new node with a given block
...
void list_add_ascending_by_address(list_t *l, block_t *newblk){
   ...
   */
    node_t *newNode = node_alloc(newblk), *current = l->head, *prev = NULL;

    // Iterate through the list to find the correct position for the new block
    ...
    if (prev == NULL) {
        l->head = newNode;  // Insert at the head if prev is NULL
    } else {
        prev->next = newNode;  // Insert after the prev node
    }
}
...
void list_add_ascending_by_blocksize(list_t *l, block_t *newblk){
   ...
   *    USE the compareSize()
   */
    int newSize = newblk->end - newblk->start + 1;  // Corrected calculation of the new block size
    node_t *newNode = node_alloc(newblk), *current = l->head, *prev = NULL;

    // Iterate through the list to find the correct position for the new block
    while (current != NULL && (current->blk->end - current->blk->start + 1) < newSize) {
        prev = current;
        current = current->next;
    }
...
void list_coalese_nodes(list_t *l){ 
   ...
   */
   if (l == NULL || l->head == NULL || l->head->next == NULL) {
        // If list is empty, has only one node, or not initialized
        return;
    }
    ...
    while (current != NULL) {
        if (prev->blk->end + 1 == current->blk->start) {
            // Blocks are adjacent, so merge them
            prev->blk->end = current->blk->end;

            // Remove current from the list and free its memory
            prev->next = current->next;
            free(current->blk); // Assuming blk was dynamically allocated
            free(current);

            // Move to next node
            current = prev->next;
        } else {
            // Blocks are not adjacent, move both pointers forward
            prev = current;
            current = current->next;
        }
    }
...
bool list_is_in_by_pid(list_t *l, int pid){ 
  ...
  // Iterate through the list to find a block with the given pid
  while (current != NULL) {
