...
void allocate_memory(list_t * freelist, list_t * alloclist, int pid, int blocksize, int policy) {
    ...
    */
    node_t *current = freelist->head, *prev = NULL, *best = NULL, *bestPrev = NULL;
    int bestSize = INT_MAX, blockSize;

    // Find a suitable block based on the policy
    while (current != NULL) {
        blockSize = current->blk->end - current->blk->start + 1;
        if (blockSize >= blocksize) {
            if (policy == 1) { // First Fit (FIFO)
                break;
            } else if (policy == 2 && blockSize < bestSize) { // Best Fit
                best = current;
                bestPrev = prev;
                bestSize = blockSize;
            } else if (policy == 3 && blockSize > bestSize) { // Worst Fit
                best = current;
                bestPrev = prev;
                bestSize = blockSize;
            }
    ...
    // Choose the block based on the policy
    if (policy != 1) {
        if (best == NULL) {
    ...
    // Allocate the memory
    block_t *allocatedBlock = current->blk;
    int originalEnd = allocatedBlock->end;
    allocatedBlock->pid = pid;
    allocatedBlock->end = allocatedBlock->start + blocksize - 1;
...
void deallocate_memory(list_t * alloclist, list_t * freelist, int pid, int policy) { 
    ...
    */
    ...
    block_t *deallocatedBlock = current->blk;
    if (prev == NULL) {
    ...
    deallocatedBlock->pid = 0;
    if (policy == 1) {
        list_add_to_back(freelist, deallocatedBlock);
    } else if (policy == 2) {
        list_add_ascending_by_blocksize(freelist, deallocatedBlock);
    } else { // policy == 3
        list_add_descending_by_blocksize(freelist, deallocatedBlock);
    }
}
